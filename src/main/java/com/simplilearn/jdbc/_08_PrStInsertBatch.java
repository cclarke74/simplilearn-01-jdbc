...
public class _08_PrStInsertBatch {
	...
	public static void main(String[] args) {
		...
		try {
			...
			users.add(new User("smmore", "12345", "Sandra", "Moore", formatter.parse("2023-09-13"), "A"));
		}
		...
		try(Connection con = DriverManager.getConnection(dbUrl, dbUsername, dbPassword);
			...
			for (User user: users) {
				 ...
				 * 
				 * We can execute prSt.executeUpdate() multiple times. One for each element
				 * on the users list, but that is inefficient. It's better to load this info in a
				 * buffer, then we create all the rows at once.
				 */
				prSt.addBatch();
			}
			
			int[] rows = prSt.executeBatch(); // Creates all the rows at once.
